...
export function initScene() {
  ...
  // Create fancy rays for the controllers using BufferGeometry
  const rayMaterial = new THREE.MeshBasicMaterial({
    color: 0xFF0000,
  });

  const rayPath = new THREE.Curve();
  rayPath.getPoint = (t) => new THREE.Vector3(0, 0, -t); // Ray extends in -Z direction

  const rayGeometry = new THREE.TubeGeometry(rayPath, 20, 0.005, 8, false); // Radius of 0.02
